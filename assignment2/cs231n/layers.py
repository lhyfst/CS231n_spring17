...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(out_height):
        for j in range(out_width):
            ...
            # 注意： 这里dx_mask 的目的是取了个dx的引用（映射），所以可以通过修改dx_mask来修改dx的对应位置的值, 这个实现太优美了
            # 另外，牢记np.array的切片是引用而不是拷贝; list的切片是拷贝不是引用
            ...
            dx_mask += flags * (dout[:, :, i, j])[:, :, None, None]  # 此处的 None 的用法是为了在特定位置增加维度
